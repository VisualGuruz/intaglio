...
var MockRepository = AbstractRepository.extend({
	_schemaPromise: null,
	_logger: console,
	_data: require('./data/data.json'),
	where: require('./where'),

	init: function () {
		// NOOP. Just Overriding the Abstract constructor.
	},
	...
	getSchema: function () {
		...
		this._schemaPromise = new RSVP.Promise(function (resolve, reject) {
			var rawSchema = require('./data/schema.json'),
	...
	find: function (model, options, conditions) {
		...
		return new RSVP.Promise(function (resolve, reject) {
			try {
				var data = this._data[model.getOriginalName()].slice(0),
					defaults = {
						limit: null,
						offset: null,
						orderBy: null,
				...
				if (combinedOptions.orderBy !== null) {
					if (combinedOptions.direction !== "descending")
						data = orderBy(data, combinedOptions.orderBy);
					else
						data = orderBy(data, combinedOptions.orderBy. true);
				}
			...
			try {
				...
				_.each(pk, function (key) {
					...
					if (data[key] === undefined || data[key] === null) {
						data[key] = _.max(self._data[model.getOriginalName()], function (item) {
							return item[key];
						})[key] + 1;
					}
				});
		...
		return new RSVP.Promise(function (resolve, reject) {
			try {
				...
				_.each(model.getPrimaryKey(), function (key) {
					conditions[key] = originalData[key];
				});

				obj = _.where(this._data[model.getOriginalName()], conditions);
	...
	delete: function (model, data) {
		...
		return new RSVP.Promise(function (resolve, reject) {
			try {
				...
				obj = _.where(this._data[model.getPrimaryKey()], conditions);
